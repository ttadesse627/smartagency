...
{
    public class ApplicantFollowupStatusController(IMediator mediator) : ApiControllerBase
    {
        ...
        {
            var response = await _mediator.Send(new CreateApplicantFollowupStatusCommand(applicantFollowupStatusRequest), token);
            return Ok(response);
        }
