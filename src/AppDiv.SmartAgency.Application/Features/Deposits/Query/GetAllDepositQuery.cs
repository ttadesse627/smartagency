...
namespace AppDiv.SmartAgency.Application.Features.Deposits.Query
{
    ...
    public class GetAllDepositHandler : IRequestHandler<GetAllDepositQuery, SearchModel<DepositResponseDTO>>
    {
        ...
        {
            var depositList = await _depositRepository.GetAllWithSearchAsync(request.PageNumber, request.PageSize, request.SearchTerm, request.OrderBy, request.SortingDirection, "Applicant");
